...
def split(img_path, output_dir_path):
    img = cv2.imread(img_path, 0)
    splitted_imges = patchify(img, (320, 320), step=320)
    name = str(Path(img_path).name)
    stem = str(Path(img_path).stem)
    subdir = output_dir_path / stem
    subdir.mkdir(parents=True, exist_ok=True)

    for i in range(splitted_imges.shape[0]):
        for j in range(splitted_imges.shape[1]):
